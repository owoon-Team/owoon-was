...
public interface RoutineExerciseResultApi {

    @Operation(summary = "운동별 실행 결과 목록 조회", description = "회원 ID와 루틴 실행 기록 ID로 운동별 실행 결과 목록을 조회합니다.")
    @ApiResponses({
    ...
    ResponseEntity<List<RoutineExerciseResultResponse>> getRoutineExerciseResults(
    ...
    @Operation(summary = "운동별 실행 결과 상세 조회", description = "회원 ID, 루틴 실행 기록 ID, 운동별 실행 결과 ID로 운동별 실행 결과를 조회합니다.")
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "조회 성공",
                    content = @Content(schema = @Schema(implementation = RoutineExerciseResultResponse.class))),
            @ApiResponse(responseCode = "404", description = "회원, 루틴 실행 기록 또는 운동별 실행 결과를 찾을 수 없음")
    })
    ResponseEntity<RoutineExerciseResultResponse> getRoutineExerciseResult(
            @Parameter(description = "회원 ID", required = true, example = "1")
            @PathVariable Long userId,
            @Parameter(description = "루틴 실행 기록 ID", required = true, example = "1")
            @PathVariable Long sessionId,
