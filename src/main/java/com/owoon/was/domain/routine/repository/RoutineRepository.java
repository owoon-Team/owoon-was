...
public interface RoutineRepository extends JpaRepository<Routine, Long> {
     ...
     */
    List<Routine> findAllByUserIdAndDeletedAtIsNullOrderByCreatedAtDesc(Long userId);

    /**
     * 회원 ID와 루틴 ID로 루틴을 조회한다.
     */
    @EntityGraph(attributePaths = {"routineExercises", "routineExercises.exercise"})
    Optional<Routine> findByIdAndUserIdAndDeletedAtIsNull(Long routineId, Long userId);
}
